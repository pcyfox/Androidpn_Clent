...
class PacketWriter {
     ...
     * 开启心跳（自定义方法）
     */
    public void startHeartBeat(int intervalTime) {
        if (heartBeat == null) heartBeat = new HeartBeatThread(intervalTime);
        if(heartBeat.isAlive())return;
    ...
    public void startHeartBeat() {
        if (heartBeat == null) heartBeat = new HeartBeatThread();
        if(heartBeat.isAlive())return;
        heartBeat.setDaemon(true);
        heartBeat.start();
    }
        ...
        public void run() {
            while (!done) {
                try {
                    writer.write(" ");
                    writer.flush();
                    Log.d(TAG, "HeartBeatThread: 心跳..." + beatTime + "次");
                    beatTime++;
                    sleep(intervalTime);
                } catch (Exception e) {
