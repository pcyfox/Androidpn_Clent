...
public class XMPPConnection extends Connection {
	...
	private void connectUsingConfiguration(ConnectionConfiguration config) throws XMPPException {
		...
		while (it.hasNext()) {
			...
			try {
				...
				exception = new XMPPException(errorMessage,
						new XMPPError(XMPPError.Condition.remote_server_error, errorMessage), ioe);
				xmppIOError = true;
			}
			if (exception == null) {
	...
	void startTLSReceived(boolean required) {
		if (required && config.getSecurityMode() == ConnectionConfiguration.SecurityMode.disabled) {
			notifyConnectionError(
					new IllegalStateException("TLS required by server but not allowed by connection configuration"));
			return;
		}
	...
	void proceedTLSReceived() throws Exception {
		...
		socket = context.getSocketFactory().createSocket(plain, plain.getInetAddress().getHostAddress(),
				plain.getPort(), true);
	 ...
	 *            心跳间隔
	 **/
	public void startHeartBeat(int intervslTime) {
		if (packetWriter != null) {
			packetWriter.startHeartBeat(intervslTime);
		}
	}
}
